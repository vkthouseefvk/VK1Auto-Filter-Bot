...
# Bot information
API_ID = environ.get('API_ID', '20581675')
if len(API_ID) == 0:
...
if len(API_HASH) == 0:
    ...
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '7652947301:AAEKfPO2IXzogeTi7u-3OnrQT7LrJyFw8VY')
if len(BOT_TOKEN) == 0:
...
# Bot Admins
ADMINS = environ.get('ADMINS', '5805236504 1454231644 6264858746')
if len(ADMINS) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1002510859195').split()]
if len(INDEX_CHANNELS) == 0:
    logger.info('INDEX_CHANNELS is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1001728230648')
...
# support group
SUPPORT_GROUP = environ.get('SUPPORT_GROUP', '-1001899852767')
...
# MongoDB information
DATA_DATABASE_URL = environ.get('DATA_DATABASE_URL', "mongodb+srv://vkthouseef3:vkthouseef3@cluster0.m7mgztp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
if len(DATA_DATABASE_URL) == 0:
    logger.error('DATA_DATABASE_URL is missing, exiting now')
    exit()
FILES_DATABASE_URL = environ.get('FILES_DATABASE_URL', "mongodb+srv://vkthouseef12:vkthouseef12@cluster0.1bgfj.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
if len(FILES_DATABASE_URL) == 0:
    logger.error('FILES_DATABASE_URL is missing, exiting now')
    exit()
SECOND_FILES_DATABASE_URL = environ.get('SECOND_FILES_DATABASE_URL', "mongodb+srv://vkthouseef4:vkthouseef4@cluster0.yu9efdk.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
URL = environ.get("URL", "-1002839501827")
if len(URL) == 0:
